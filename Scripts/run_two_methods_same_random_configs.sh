...
#   4) mip batch (via Scripts/run_all_mip_configs.sh / mip_eval.py)
#
# Usage:
#   ./Scripts/run_two_methods_same_random_configs.sh
#
...
launch_gnn_batch() {
  ...
  nohup env \
    ...
    ./Scripts/run_all_gnn_configs.sh > "$log_file" 2>&1 &
  echo $!
}
...
nohup env \
  ...
  OUTDIR="$MIP_OUTDIR" \
  ./Scripts/run_all_mip_configs.sh > "$MIP_LOG" 2>&1 &
MIP_PID=$!
...
echo "  CONFIG_GLOB=\"$CONFIG_GLOB_VALUE\" SOLVER_TOOL=\"$MIP_SOLVER_TOOL\" MIP_EVAL_PY=\"$MIP_EVAL_PY\" OUTDIR=\"$MIP_OUTDIR\" ./Scripts/run_all_mip_configs.sh"
