...
class TaskGraph:
    ...
    def load_graph_from_pydot(self, pydot_file, k=1.5, l=0.2, mu=0.5, A_max=100, seed=42, reproduce=True):
        ...
        # Populate legacy NetworkX attributes used by downstream utilities.
        self.legacy_graph = self.graph
        nx.set_node_attributes(self.legacy_graph, self.hardware_area, 'area_cost')
        nx.set_node_attributes(self.legacy_graph, self.hardware_costs, 'hardware_time')
        nx.set_node_attributes(self.legacy_graph, self.software_costs, 'software_time')
        nx.set_edge_attributes(self.legacy_graph, self.communication_costs, 'communication_cost')
    ...
    def evaluate_makespan(self, partition_assignment, verbose = False):
        ...
        Adapted from the legacy scheduler path in utils/scheduler_utils.py
